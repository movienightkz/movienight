...
$db = mysqli_connect('localhost', 'mysql', 'mysql', 'registration');
...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($username)) { array_push($errors, "Требуется ввести имя"); }
  if (empty($email)) { array_push($errors, "Требуется ввести email"); }
...
if (isset($_POST['login_user'])) {
  ...
  if (empty($username)) {
    array_push($errors, "Требуется ввести имя");
  }
  if (empty($password)) {
  ...
  if (count($errors) == 0) {
    ...
    if (mysqli_num_rows($results) == 1) {
      ...
      array_push($errors, "Неверный пароль/имя ползьователя");
    }
  }
}
